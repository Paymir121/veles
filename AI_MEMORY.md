...
- **`main_id` по умолчанию = `data[0].id`**, т.е. зависит от порядка, в котором backend отдаёт
  ...
  потомков. Библиотека всё равно рисует только ОДНУ кровную линию (лес не умеет), поэтому на
  странице дерева есть переключатель «Ветка семьи» по всем людям без родителей
  (`findFamilyIslands`) — иначе чужие семьи / второй брак примака остаются только в поиске.
- **Известное огр.**: если у "примака" есть ВТОРОЙ брак/ребёнок от другого партнёра (Виктор →
  Ольга умерла → Татьяна, Роман) — эта ветка не попадает в окно вокруг Соколовых. Переключатель
  ветки (или поиск + `updateMainId`) показывает её, ценой пересчёта окна от нового `main_id`.
